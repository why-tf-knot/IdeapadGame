...
interface Props {
  route: { params: { threadId: string; ideaTitle?: string; founderName?: string } };
  navigation: any;
}

export default function ChatScreen({ route, navigation }: Props) {
  const { threadId, ideaTitle, founderName } = route.params;
  ...
  useEffect(() => {
    navigation.setOptions({
      title: founderName ? `Chat with ${founderName}` : ideaTitle || 'Messages',
    });
    loadUserId();
    fetchMessages();
    connectSocket();
  ...
  const renderMessage = ({ item }: { item: ChatMessage }) => {
    ...
    return (
      <View style={[styles.bubble, isMine ? styles.myBubble : styles.theirBubble]}>
        <Text style={[styles.roleLabel, { color: isMine ? '#F59E0B' : COLORS.accent }]}>
          {item.senderRole === 'INVESTOR' ? 'You (Investor)' : founderName || 'Founder'}
        </Text>
