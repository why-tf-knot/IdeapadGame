...
export default function ThreadsListScreen({ navigation }: Props) {
  ...
  const getInvestorName = (thread: ChatThread) => {
    if (typeof thread.investorId === 'object' && thread.investorId?.name) {
      return thread.investorId.name;
    }
    return 'Investor';
  };
    ...
    <TouchableOpacity
      ...
      onPress={() =>
        navigation.navigate('Chat', {
          threadId: item._id,
          investorName: getInvestorName(item),
        })
      }
    >
      <View style={styles.threadHeader}>
        <Text style={styles.threadName}>{getInvestorName(item)}</Text>
        {item.founderUnread > 0 && (
          <View style={styles.badge}>
            <Text style={styles.badgeText}>{item.founderUnread}</Text>
          </View>
        )}
      </View>
        ...
        ListEmptyComponent={
          <View style={styles.center}>
            ...
            <Text style={styles.emptySubtext}>
              Investors will be able to message you about your ideas
            </Text>
          </View>
        }
      />
